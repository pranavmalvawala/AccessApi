...
export class RoleMemberRepository {
    ...
    public loadByRoleId(roleId: string, churchId: string): Promise<RoleMember[]> {
        return DB.query("SELECT rm.*, uc.personId FROM roleMembers rm LEFT JOIN userchurches uc ON rm.userId=uc.userId WHERE roleId=? AND rm.churchId=?;", [roleId, churchId]);
    }
